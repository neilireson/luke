...
public class Thinlet extends Container implements Runnable, Serializable {
    ...
    public Thinlet() {
        ...
        try {
            if ((System.getProperty("java.vendor").contains("Insignia"))
                    && !System.getProperty("os.name").contains("Windows CE")) {
                evm = -1;
            }
        } catch (Exception exc) { /* never */
        }
    ...
    public void doLayout(Object component) {
        ...
        if ("combobox" == classname) {
            ...
            layoutScroll(component, width + 2, height - fm.getLeading() + 2, 0, 0, 0, 0, getBoolean(component,
                    "border", true), 0);
            ...
            for (Object tab = get(component, ":comp"); tab != null; tab = get(tab, ":next")) {
                ...
                setRectangle(tab, "bounds", horizontal ? tabd : 0, horizontal ? 0 : tabd, stacked ? bounds.width
                        : d.width, d.height);
                if (stacked) {
                    tabd += d.height;
                } else {
            ...
            // adjust bounds of the tabbedpane
            if (lastTab != null) {
              boolean adjust = false;
              Rectangle tb = getRectangle(lastTab, "bounds");
              if (tb.x + tb.width > bounds.x + bounds.width) {
                bounds.width = tb.x + tb.width + 2 - bounds.x;
                adjust = true;
              }
              if (tb.y + tb.height > bounds.y + bounds.height) {
                bounds.height = tb.y + tb.height + 2 - bounds.y;
                adjust = true;
              }
//              if (adjust) {
//                setRectangle(component, "bounds", bounds.x, bounds.y, bounds.width, bounds.height);
//                System.out.println("lastTab=" + tb + ", tabpane=" + bounds);
//                System.err.println("adjust " + tb);
//              }
            }
            ...
            for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                ...
                if (iclass == "dialog") {
                    ...
                    if (get(comp, "bounds") == null)
                            setRectangle(comp, "bounds", Math.max(0, (bounds.width - d.width) / 2), Math.max(0,
                                    (bounds.height - d.height) / 2), Math.min(d.width, bounds.width), Math.min(
            ...
            if (visible1) {
                setRectangle(comp1, "bounds", 0, 0, horizontal ? divider : bounds.width, horizontal ? bounds.height
                        : divider);
                doLayout(comp1);
            }
            Object comp2 = (comp1 != null) ? get(comp1, ":next") : null;
            if ((comp2 != null) && getBoolean(comp2, "visible", true)) {
                setRectangle(comp2, "bounds", horizontal ? (divider + 5) : 0, horizontal ? 0 : (divider + 5),
                        horizontal ? (bounds.width - 5 - divider) : bounds.width, horizontal ? bounds.height
     ...
     *  
     */
    private char[] getChars(Object component, String text, boolean wrap, int width, int height) {
        ...
        if (wrap) {
            ...
            for (int i = 0, j = -1, k = 0; k <= n; k++) { // j is the last space
                ...
                // (before k)
                if (((k == n) || (chars[k] == '\n') || (chars[k] == ' ')) && (j > i)
                        && (fm.charsWidth(chars, i, k - i) > width)) {
                    chars[j] = '\n';
    ...
    private String getAccelerator(Object component) {
        ...
        if (accelerator != null) {
            long keystroke = (Long) accelerator;
    ...
    private Object popupMenu(Object component) {
        ...
        popup(selected, popup, menubar ? (("bottom" != get(component, "placement")) ? 'D' : 'U') : 'R',
                menubar ? (menux + menubounds.x) : menux, menuy + menubounds.y, menubar ? menubounds.width : menuwidth,
     ...
     * 
     * @param menu menubar's menu, menu's menu, or component's popupmenu
     *        including items
     ...
     */
    private void popup(Object menu, Object popup, char direction, int x, int y, int width, int height, int offset) {
        int pw = 0;
        int ph = 0;
        for (Object item = get(menu, ":comp"); item != null; item = get(item, ":next")) {
            ...
            if (accelerator != null) {
                d.width += 4 + getFontMetrics(font).stringWidth(accelerator); //TODO
                // font,
                // height
                // and gap
            }
        ...
        if (direction == 'R') {
            ...
            y += ((direction == 'U') ? (topspace || !bottomspace) : (!bottomspace && topspace)) ? (offset - ph)
                    : (height - offset);
        }
        setRectangle(popup, "bounds", Math.max(0, Math.min(x, desktop.width - pw)), Math.max(0, Math.min(y,
                desktop.height - ph)), pw, ph);
        repaint(popup);
    }
    ...
    private void layoutField(Object component, int dw, boolean hidden, int left) {
        ...
        off = Math.max(0, Math.min(off, (hidden ? (fm.charWidth('*') * text.length()) : fm.stringWidth(text)) - width
                + 4));
        if (off != offset) {
     ...
     * @param top top inset (e.g. table header, dialog title, half of panel
     *        title)
     ...
     */
    private boolean layoutScroll(Object component, int contentwidth, int contentheight, int top, int left, int bottom,
            int right, boolean border, int topgap) {
        Rectangle bounds = getRectangle(component, "bounds");
        int iborder = border ? 1 : 0;
        ...
        if (hneed) {
            setRectangle(component, ":horizontal", left, bounds.height - bottom - block - 1, bounds.width - left
                    - right - (vneed ? block : 0), block + 1);
        } else {
            set(component, ":horizontal", null);
        }
        if (vneed) {
            setRectangle(component, ":vertical", bounds.width - right - block - 1, top, block + 1, bounds.height - top
                    - bottom - (hneed ? block : 0));
        } else {
     ...
     *  
     */
    private void scrollToVisible(Object component, int x, int y, int width, int height) {
        Rectangle view = getRectangle(component, ":view");
    ...
    protected Dimension getPreferredSize(Object component) {
        ...
        if ("tabbedpane" == classname) {
            ...
            for (Object tab = get(component, ":comp"); tab != null; tab = get(tab, ":next")) {
                ...
                if (placement == "stacked") {
                    ...
                    listLen += horizontal ? d.width + 9 : d.height + 5;
                }
            ...
            if (horizontal) {
              if (listLen > contentwidth) contentwidth = listLen;
            } else {
              if (listLen > contentheight) contentheight = listLen;
            }
            return new Dimension(contentwidth + (horizontal ? 4 : (tabsize + 3)), contentheight
        ...
        if ("splitpane" == classname) {
            boolean horizontal = ("vertical" != get(component, "orientation"));
            Object comp1 = get(component, ":comp");
            Dimension size = ((comp1 == null) || !getBoolean(comp1, "visible", true)) ? new Dimension()
                    : getPreferredSize(comp1);
    ...
    private Dimension getFieldSize(Object component) {
        ...
        Font currentfont = (Font) get(component, "font");
        FontMetrics fm = getFontMetrics((currentfont != null) ? currentfont : font);
        return new Dimension(((columns > 0) ? (columns * fm.charWidth('e')) : 76) + 4, fm.getAscent() + fm.getDescent()
                + 4); // fm.stringWidth(text)
    }
     ...
     *         offsets
     */
    public Dimension getSize(Object component, int dx, int dy) {
    ...
    /**
     * @param clipx the cliping rectangle is relative to the component's parent
     ...
     * @throws java.lang.IllegalArgumentException
     */
    private void paint(Graphics g, int clipx, int clipy, int clipwidth, int clipheight, Object component,
            boolean enabled) {
        ...
        if ((clipx + clipwidth < bounds.x) || (clipx > bounds.x + bounds.width) || (clipy + clipheight < bounds.y)
        ...
        if ("label" == classname) {
            paint(component, 0, 0, bounds.width, bounds.height, g, clipx, clipy, clipwidth, clipheight, false, false,
                    false, false, 0, 0, 0, 0, false, enabled ? 'e' : 'd', "left", true, false);
            ...
            boolean link = ("button" == classname) && (get(component, "type") == "link");
            if (link) {
                paint(component, 0, 0, bounds.width, bounds.height, g, clipx, clipy, clipwidth, clipheight, false,
                        false, false, false, 0, 0, 0, 0, focus, enabled ? (pressed ? 'e' : 'l') : 'd', "center", true,
                        enabled && (inside != pressed));
            } else { // disabled toggled
                char mode = enabled ? ((inside != pressed) ? 'h' : ((pressed || toggled) ? 'p' : 'g')) : 'd';
                paint(component, 0, 0, bounds.width, bounds.height, g, clipx, clipy, clipwidth, clipheight, true, true,
                        true, true, 2, 5, 2, 5, focus, mode, "center", true, false);
            ...
            paint(component, 0, 0, bounds.width, bounds.height, g, clipx, clipy, clipwidth, clipheight, false, false,
                    false, false, 0, block + 3, 0, 0, false, enabled ? 'e' : 'd', "left", true, false);
            ...
            if (focus) {
                ...
                if (group == null) {
                    ...
                    g.drawRect(1 + dd - 2, 1 + dy + dd - 2, dd + 4, dd + 4);
                }
            }
            if ((!selected && inside && pressed) || (selected && (!inside || !pressed))) {
                ...
                if (group == null) {
                    ...
                    g.fillRect(1 + dd, 1 + dy + dd, dd + 1, dd + 1);
            ...
            if (getBoolean(component, "editable", true)) {
                ...
                paintField(g, clipx, clipy, clipwidth, clipheight, component, bounds.width - block, bounds.height,
                        focus, enabled, false, left);
                if (icon != null) {
                    g.drawImage(icon, 2, (bounds.height - icon.getHeight(this)) / 2, this);
                }
                paintArrow(g, bounds.width - block, 0, block, bounds.height, 'S', enabled, inside, pressed, "down",
                        true, false, true, true, true);
            } else {
                paint(component, 0, 0, bounds.width, bounds.height, g, clipx, clipy, clipwidth, clipheight, true, true,
                        true, true, 1, 1, 1, 1 + block, focus, enabled ? ((inside != pressed) ? 'h' : (pressed ? 'p'
                                : 'g')) : 'd', "left", false, false);
                g.setColor(enabled ? c_text : c_disable);
                paintArrow(g, bounds.width - block, 0, block, bounds.height, 'S');
            }
        } else if (":combolist" == classname) {
            paintScroll(component, classname, pressed, inside, focus, enabled, g, clipx, clipy, clipwidth, clipheight);
        } else if (("textfield" == classname) || ("passwordfield" == classname)) {
            paintField(g, clipx, clipy, clipwidth, clipheight, component, bounds.width, bounds.height, focus, enabled,
            ...
            for (Object tab = get(component, ":comp"); tab != null; tab = get(tab, ":next")) {
                ...
                if (r.x + r.width > bounds.x + bounds.width) {
                  bounds.width = r.x + r.width + 2 - bounds.x;
                  adjust = true;
                }
                if (r.y + r.height > bounds.y + bounds.height) {
                  bounds.height = r.y + r.height + 2 - bounds.y;
                  adjust = true;
                }
                if (adjust) {
                  setRectangle(component, "bounds", bounds.x, bounds.y, bounds.width, bounds.height);
                  System.err.println("adjust for " + r);
                }
                if (selected != i) {
                    boolean hover = inside && (mousepressed == null) && (insidepart == tab);
                    boolean tabenabled = enabled && getBoolean(tab, "enabled", true);
                    paint(tab, r.x + bx, r.y + by, r.width - bw, r.height - bh, g, clipx, clipy, clipwidth, clipheight,
                            (placement != "bottom"), (placement != "right"), !stacked && (placement != "top"),
                            (placement != "left"), 1, 3, 1, 3, false, tabenabled ? (hover ? 'h' : 'g') : 'd', "left",
                            true, false);

                } else {
                    selectedtab = tab;
                    // paint tabbedpane border
                    paint(tab, (placement == "left") ? r.width - 1 : 0, stacked ? (r.y + r.height - 1)
                            : (placement == "top") ? r.height - 1 : 0, (horizontal || stacked) ? bounds.width
                            : (bounds.width - r.width + 1), stacked ? (bounds.height - r.y - r.height + 1)
                            : horizontal ? (bounds.height - r.height + 1) : bounds.height, g, true, true, true, true,
                            enabled ? 'e' : 'd');
            ...
            if (selectedtab != null) {
                ...
                paint(selectedtab, r.x, r.y, r.width, r.height, g, clipx, clipy, clipwidth, clipheight,
                        (placement != "bottom"), (placement != "right"), !stacked && (placement != "top"),
                        (placement != "left"), pv, ph, pv, ph, focus, enabled ? 'b' : 'i', "left", true, false);
            }
            ...
            if ("dialog" == classname) {
                paint(component, 0, 0, bounds.width, 3 + titleheight, g, clipx, clipy, clipwidth, clipheight, true,
                ...
                if (get(component, "close") != null) {
                    paint(component, g, controlx, 3, titleheight - 2, titleheight - 2, 'c',
                            pressedpart == ":closebutton" ? 'p' : insidepart == ":closebutton" ? 'h' : 'g');
                ...
                paintRect(g, 0, 3 + titleheight, bounds.width, bounds.height - 3 - titleheight, c_border, c_press,
                        false, true, true, true, true); // lower
                // part
                // excluding
                // titlebar
                paint(component, // content area
                        3, 3 + titleheight, bounds.width - 6, bounds.height - 6 - titleheight, g, true, true, true,
                        true, 'b');
            } else { // panel
                boolean border = getBoolean(component, "border", false);
                paint(component, 0, titleheight / 2, bounds.width, bounds.height - (titleheight / 2), g, border,
                        border, border, border, enabled ? 'e' : 'd');
                paint(component, 0, 0, bounds.width, titleheight, // panel title
                        g, clipx, clipy, clipwidth, clipheight, false, false, false, false, 0, 3, 0, 3, false,
                        enabled ? 'x' : 'd', "left", false, false);
            }

            if (get(component, ":port") != null) {
                paintScroll(component, classname, pressed, inside, focus, enabled, g, clipx, clipy, clipwidth,
                        clipheight);
            } else {
                for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                    paint(g, clipx, clipy, clipwidth, clipheight, comp, enabled);
            ...
            if ((tooltipowner != null) && (component == content)) {
                ...
                if (r != null) {
                  paintRect(g, r.x, r.y, r.width, r.height, c_border, c_hover, true, true, true, true, true);
            ...
            paintField(g, clipx, clipy, clipwidth, clipheight, component, bounds.width - block, bounds.height, focus,
                    enabled, false, 0);
            paintArrow(g, bounds.width - block, 0, block, bounds.height / 2, 'N', enabled, inside, pressed, "up", true,
                    false, false, true, true);
            paintArrow(g, bounds.width - block, bounds.height / 2, block, bounds.height - (bounds.height / 2), 'S',
                    enabled, inside, pressed, "down", true, false, true, true, true);
            ...
            paintRect(g, 0, 0, horizontal ? length : bounds.width, horizontal ? bounds.height : length,
                    enabled ? c_border : c_disable, c_select, true, true, horizontal, !horizontal, true);
            paintRect(g, horizontal ? length : 0, horizontal ? 0 : length, horizontal ? (bounds.width - length)
                    : bounds.width, horizontal ? bounds.height : (bounds.height - length), enabled ? c_border
            ...
            int value = getInteger(component, "value", 0);
            boolean horizontal = ("vertical" != get(component, "orientation"));
            int length = (value - minimum) * ((horizontal ? bounds.width : bounds.height) - block)
                    / (maximum - minimum);
            paintRect(g, horizontal ? 0 : 3, horizontal ? 3 : 0, horizontal ? length : (bounds.width - 6),
                    horizontal ? (bounds.height - 6) : length, enabled ? c_border : c_disable, c_bg, true, true,
                    horizontal, !horizontal, true);
            paintRect(g, horizontal ? length : 0, horizontal ? 0 : length, horizontal ? block : bounds.width,
                    horizontal ? bounds.height : block, enabled ? c_border : c_disable, enabled ? c_ctrl : c_bg, true,
                    true, true, true, true);
            if (focus) {
                g.setColor(c_focus);
                g.drawRect(horizontal ? (length + 2) : 2, horizontal ? 2 : (length + 2), (horizontal ? block
                        : bounds.width) - 5, (horizontal ? bounds.height : block) - 5);
                //g.drawRect(length + 1, 1, block - 3, bounds.height - 3);
            }
            paintRect(g, horizontal ? (block + length) : 3, horizontal ? 3 : (block + length), bounds.width
                    - (horizontal ? (block + length) : 6), bounds.height - (horizontal ? 6 : (block + length)),
                    enabled ? c_border : c_disable, c_bg, horizontal, !horizontal, true, true, true);
        } else if ("splitpane" == classname) {
            boolean horizontal = ("vertical" != get(component, "orientation"));
            int divider = getInteger(component, "divider", -1);
            paintRect(g, horizontal ? divider : 0, horizontal ? 0 : divider, horizontal ? 5 : bounds.width,
                    horizontal ? bounds.height : 5, c_border, c_bg, false, false, false, false, true);
            ...
            for (Object menu = get(component, ":comp"); menu != null; menu = get(menu, ":next")) {
                ...
                paint(menu, mb.x, 0, mb.width, bounds.height, g, clipx, clipy, clipwidth, clipheight, // TODO
                                                                                                      // disabled
                        bottom || armed, armed, !bottom || armed, armed, 1, 3, 1, 3, false,
                        enabled ? (menuenabled ? (armed ? 's' : (hoover ? 'h' : 'g')) : 'r') : 'd', "left", true, false);
                lastx = mb.x + mb.width;
            }
            paintRect(g, lastx, 0, bounds.width - lastx, bounds.height, enabled ? c_border : c_disable,
                    enabled ? c_ctrl : c_bg, bottom, false, !bottom, false, true);
            ...
            for (Object menu = get(get(component, "menu"), ":comp"); menu != null; menu = get(menu, ":next")) {
                ...
                if (itemclass == "separator") {
                    ...
                    paint(menu, r.x, r.y, bounds.width - 2, r.height, g, clipx, clipy, clipwidth, clipheight, false,
                            false, false, false, 2, (itemclass == "checkboxmenuitem") ? (block + 7) : 4, 2, 4, false,
                    ...
                    if (itemclass == "menu") {
                        ...
                        if (accelerator != null) { //TODO
                            g.drawString(accelerator, bounds.width - 4 - getFontMetrics(font).stringWidth(accelerator),
                                    r.y + 2 + 10);
                        }
                    }
                }
     ...
     *  
     */
    private void paintReverse(Graphics g, int clipx, int clipy, int clipwidth, int clipheight, Object component,
            boolean enabled) {
        if (component != null) {
            Rectangle bounds = getRectangle(component, "bounds");
            if ((clipx < bounds.x) || (clipx + clipwidth > bounds.x + bounds.width) || (clipy < bounds.y)
    ...
    private void paintField(Graphics g, int clipx, int clipy, int clipwidth, int clipheight, Object component,
        ...
        if (focus) {
            if (start != end) {
                ...
                g.setColor(c_select);
                g.fillRect(fx + Math.min(is, caret), 1, Math.abs(caret - is) + evm, height - 2 + evm);
            }
    ...
    private void paintScroll(Object component, String classname, boolean pressed, boolean inside, boolean focus,
            boolean enabled, Graphics g, int clipx, int clipy, int clipwidth, int clipheight) {
        Rectangle port = getRectangle(component, ":port");
        ...
        if (horizontal != null) { // paint horizontal scrollbar
            ...
            paintArrow(g, x + width - block, y, block, height, 'E', enabled, inside, pressed, "right", true, false,
                    true, true, true);

            int track = width - (2 * block);
            if (track < 10) {
                paintRect(g, x + block, y, track, height, enabled ? c_border : c_disable, c_bg, true, true, true, true,
                        true);
            } else {
                int knob = Math.max(track * port.width / view.width, 10);
                int decrease = view.x * (track - knob) / (view.width - port.width);
                paintRect(g, x + block, y, decrease, height, enabled ? c_border : c_disable, c_bg, false, true, true,
                        false, true);
                paintRect(g, x + block + decrease, y, knob, height, enabled ? c_border : c_disable, enabled ? c_ctrl
                        : c_bg, true, true, true, true, true);
                ...
                paintRect(g, x + block + decrease + knob, y, increase, height, enabled ? c_border : c_disable, c_bg,
        ...
        if (vertical != null) { // paint vertical scrollbar
            ...
            paintArrow(g, x, y + height - block, width, block, 'S', enabled, inside, pressed, "down", false, true,
                    true, true, false);

            int track = height - (2 * block);
            if (track < 10) {
                paintRect(g, x, y + block, width, track, enabled ? c_border : c_disable, c_bg, true, true, true, true,
                        false);
            } else {
                int knob = Math.max(track * port.height / view.height, 10);
                int decrease = view.y * (track - knob) / (view.height - port.height);
                paintRect(g, x, y + block, width, decrease, enabled ? c_border : c_disable, c_bg, true, false, false,
                        true, false);
                paintRect(g, x, y + block + decrease, width, knob, enabled ? c_border : c_disable, enabled ? c_ctrl
                        : c_bg, true, true, true, true, false);
                int n = Math.min(5, (knob - 4) / 3);
                g.setColor(enabled ? c_border : c_disable);
                ...
                int increase = track - decrease - knob;
                paintRect(g, x, y + block + decrease + knob, width, increase, enabled ? c_border : c_disable, c_bg,
            ...
            if ("table" == classname) {
                ...
                if (header != null) {
                    ...
                    for (int i = 0; i < columnwidths.length; i++) {
                        ...
                        // or disabled
                        // (d)
                        paint(column, x - view.x, 0, width, port.y - 1, g, clipx, clipy, clipwidth, clipheight, true,
                                true, false, lastcolumn, 1, 1, 0, 0, false, enabled ? (pressedpart == column ? 'p'
                                        : (insidepart == column ? 'h' : (getBoolean(column, "selected") ? 'b' : 'g')))
                                        : 'd', "left", false, false);
        ...
        if ((x2 > x1) && (y2 > y1)) {
            ...
            paint(component, classname, focus, enabled, g, view.x - port.x + x1, view.y - port.y + y1, x2 - x1,
                    y2 - y1, port.width, view.width);
     ...
     */
    private void paint(Object component, String classname, boolean focus, boolean enabled, Graphics g, int clipx,
        ...
        if ("textarea" == classname) {
            ...
            for (Object choice = get(get(component, "combobox"), ":comp"); choice != null; choice = get(choice, ":next")) {
                ...
                paint(choice, r.x, r.y, portwidth, r.height, g, clipx, clipy, clipwidth, clipheight, false, false,
                        false, false, 2, 4, 2, 4, false, getBoolean(choice, "enabled", true) ? ((lead == choice) ? 's'
                                : 't') : 'd', "left", false, false);
            }
        } else if (("panel" == classname) || ("dialog" == classname)) {
            for (Object comp = get(component, ":comp"); comp != null; comp = get(comp, ":next")) {
                paint(g, clipx, clipy, clipwidth, clipheight, comp, enabled);
            ...
            for (Object item = get(component, ":comp"), next = null; item != null; item = next) {
                ...
                paintRect(g, ("tree" != classname) ? 0 : r.x, r.y, ("tree" != classname) ? viewwidth : r.width,
                        r.height, c_focus, selected ? c_select : c_textbg, focused, focused, focused, focused, true);
                if (line) {
                    g.setColor(c_bg);
                ...
                if ("table" != classname) { // list or tree
                    ...
                    paint(item, r.x, r.y, viewwidth, r.height, g, clipx, clipy, clipwidth, clipheight, false, false,
                            false, false, 1, 3, 1, 3, false, itemenabled ? 'e' : 'd', "left", false, false);
                    if ("tree" == classname) {
                        int x = r.x - block / 2;
                        ...
                        if (subnode) {
                            paintRect(g, x - 4, y - 4, 9, 9, itemenabled ? c_border : c_disable, itemenabled ? c_ctrl
                                    : c_bg, true, true, true, true, true);
     ...
     *  
     */
    private void paintRect(Graphics g, int x, int y, int width, int height, Color border, Color bg, boolean top,
    ...
    private void fill(Graphics g, int x, int y, int width, int height, boolean horizontal) {
        if (horizontal) {
            ...
            for (int i = 0; i < width; i += block) {
                g.drawImage(hgradient, x + i, (height > block) ? (y + height - block) : y, x
                        + Math.min(i + block, width) + evm, y + height + evm, 0, 0, Math.min(block, width - i) + evm,
                        Math.min(block, height) + evm, null);
            }
        } else {
            if (width > block) {
                g.setColor(c_bg);
            ...
            for (int i = 0; i < height; i += block) {
                g.drawImage(vgradient, (width > block) ? (x + width - block) : x, y + i, x + width + evm, y
                        + Math.min(i + block, height) + evm, 0, 0, Math.min(block, width) + evm, Math.min(block, height
                        - i)
                        + evm, null);
            }
        }
     ...
     *  
     */
    private void paintArrow(Graphics g, int x, int y, int width, int height, char dir, boolean enabled, boolean inside,
        ...
        paintRect(g, x, y, width, height, enabled ? c_border : c_disable, enabled ? ((inside != pressed) ? c_hover
                : (pressed ? c_press : c_ctrl)) : c_bg, top, left, bottom, right, horizontal);
        g.setColor(enabled ? c_text : c_disable);
        paintArrow(g, x + (left ? 1 : 0), y + (top ? 1 : 0), width - (left ? 1 : 0) - (right ? 1 : 0), height
                - (top ? 1 : 0) - (bottom ? 1 : 0), dir);
    }
     ...
     */
    private void paint(Object component, int x, int y, int width, int height, Graphics g, boolean top, boolean left,
        ...
        switch (mode) {
            ...
            case 'x':
                ...
                break;
            case 'h':
                background = (background != null) ? background.brighter() : c_hover;
                break;
            case 'p':
                background = (background != null) ? background.darker() : c_press;
                break;
            case 't':
                if (background == null) {
                    background = c_textbg;
     ...
     */
    private void paint(Object component, Graphics g, int x, int y, int width, int height, char type, char mode) {
     ...
     */
    private void paint(Object component, int x, int y, int width, int height, Graphics g, int clipx, int clipy,
        ...
        if (text != null) {
            ...
            if (foreground == null) {
                foreground = (mode == 'l') ? Color.blue : (((mode != 'd') && (mode != 'r')) ? c_text : c_disable);
            }
...
//    }
    ...
    protected void processEvent(AWTEvent e) {
        ...
        int id = e.getID();
        if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_MOVED) || (id == MouseEvent.MOUSE_EXITED)
            ...
            if (id == MouseEvent.MOUSE_ENTERED) {
                if (mousepressed == null) {
                    ...
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, MouseEvent.MOUSE_ENTERED,
                ...
                if ((previnside == mouseinside) && (prevpart == insidepart)) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, MouseEvent.MOUSE_MOVED,
                            mouseinside, insidepart);
                } else {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, MouseEvent.MOUSE_EXITED,
                            previnside, prevpart);
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, MouseEvent.MOUSE_ENTERED,
                            mouseinside, insidepart);
                }
                ...
                handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, MouseEvent.MOUSE_PRESSED,
                ...
                if (wasin && !isin) {
                    ...
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, DRAG_EXITED, previnside,
                            prevpart);
                }
                if (isin && !wasin) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, MouseEvent.MOUSE_ENTERED,
                    ...
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, DRAG_ENTERED, mouseinside,
                            insidepart);
                }
                if (isin == wasin) {
                    handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, MouseEvent.MOUSE_DRAGGED,
                            mousepressed, pressedpart);
                }
                ...
                handleMouseEvent(x, y, clickcount, shiftdown, controldown, popuptrigger, MouseEvent.MOUSE_RELEASED,
            ...
            if (port != null) { // is scrollable
                ...
                try { // mouse wheel is supported since 1.4 thus it use
                    ...
                    int rotation = (Integer) wheelrotation.invoke(e, (Object[]) null);

                    if (port.x + port.width < bounds.width) { // has vertical
                                                              // scrollbar
                        processScroll(mouseinside, (rotation > 0) ? "down" : "up"); //TODO
                        // scroll
                        // panels
                        // too
            ...
            if (focusinside && ((popupowner != null) || (focusowner != null))) {
                ...
                if ((control == (id == KeyEvent.KEY_PRESSED))
                    ...
                    if (!ke.isShiftDown() ? setNextFocusable(focusowner, outgo) : setPreviousFocusable(focusowner,
                            outgo)) {
                        ke.consume();
     ...
     *  
     */
    private boolean processKeyPress(Object component, boolean shiftdown, boolean controldown, int modifiers,
        ...
        if ("button" == classname) {
            ...
            if (combolist == null) { // the drop down list is not visible
                boolean editable = getBoolean(component, "editable", true);
                if (editable
                        && processField(component, shiftdown, controldown, modifiers, keychar, keycode, false, false,
                                false)) {
                ...
                if ((keycode == KeyEvent.VK_UP) || (keycode == KeyEvent.VK_DOWN) || (keycode == KeyEvent.VK_PAGE_UP)
                        || (keycode == KeyEvent.VK_PAGE_DOWN) || (keycode == KeyEvent.VK_HOME)
                ...
                } else if (!processField(component, shiftdown, controldown, modifiers, keychar, keycode, false, false,
                        false)) {
            ...
            return processField(component, shiftdown, controldown, modifiers, keychar, keycode, false,
                    ("passwordfield" == classname), false);
        } else if ("textarea" == classname) {
            char[] chars = (char[]) get(component, ":text");
            int start = getInteger(component, "start", 0);
            int end = getInteger(component, "end", 0);
            ...
            } else return processField(component, shiftdown, controldown, modifiers, keychar, keycode, true, false,
                    false);
            ...
            return processField(component, shiftdown, controldown, modifiers, keychar, keycode, false, false, true);
        } else if ("slider" == classname) {
            int value = getInteger(component, "value", 0);
            int d = 0;
            if ((keycode == KeyEvent.VK_HOME) || (keycode == KeyEvent.VK_LEFT) || (keycode == KeyEvent.VK_UP)
    ...
    private Object getMenu(Object component, Object part, boolean forward, boolean popup) {
        ...
        for (int i = 0; i < 2; i++) { // 0: next to last, 1: first to previous
            for (Object item = (i == 0) ? get(part, ":next") : get(popup ? get(component, "menu") : component, ":comp"); (i == 0) ? (item != null)
     ...
     */
    private boolean processField(Object component, boolean shiftdown, boolean controldown, int modifiers, int keychar,
            int keycode, boolean multiline, boolean hidden, boolean filter) {
        String text = getString(component, "text", "");
        int start = getInteger(component, "start", 0);
        int end = getInteger(component, "end", 0);
        boolean editable = getBoolean(component, "editable", true);
        ...
        //((modifiers == 0) || (modifiers == InputEvent.SHIFT_MASK))) {
            ...
            try {
                insert = (String) getToolkit().getSystemClipboard().getContents(this).getTransferData(
    ...
    /**
     * @param component a textfield, passwordfield, textarea, combobox, or
     *        spinbox
     ...
     */
    protected boolean changeField(Object component, String text, String insert, int movestart, int moveend, int start,
            int end) {
     ...
     *  
     */
    private boolean processList(Object component, boolean shiftdown, boolean controldown, int keychar, int keycode,
            boolean recursive) {
        ...
        if (keychar != 0) {
            ...
            for (; i < 2; i++) { // 0: find the long text, 1: the stating
                ...
                for (int j = 0; j < 2; j++) { // 0: lead to last, 1: first to
                                              // lead
                    for (Object item = (j == 0) ? ((i == 0) ? lead : getNextItem(component, lead, recursive)) : get(
                            component, ":comp"); (j == 0) ? (item != null) : (item != lead); item = getNextItem(
     ...
     *  
     */
    private void extend(Object component, Object lead, Object row, boolean recursive) {
     ...
     */
    private void handleMouseEvent(int x, int y, int clickcount, boolean shiftdown, boolean controldown,
            boolean popuptrigger, int id, Object component, Object part) {
        if (id == MouseEvent.MOUSE_ENTERED) {
            setTimer(750L);
        } else if (id == MouseEvent.MOUSE_EXITED) {
            hideTip();
        ...
        if (("list" == classname) || ("table" == classname) || ("tree" == classname)) {
            ...
            if (header != null && get(header, "action") != null) {
                if (insidepart != null && insidepart instanceof Object[] && "column" == getClass(insidepart)) {
                    noScroll = true;
                    if (id == MouseEvent.MOUSE_ENTERED || id == MouseEvent.MOUSE_PRESSED
                            || id == MouseEvent.MOUSE_RELEASED) {
            ...
            if (id == MouseEvent.MOUSE_PRESSED || id == MouseEvent.MOUSE_RELEASED || id == MouseEvent.MOUSE_EXITED
                    || id == MouseEvent.MOUSE_ENTERED || id == MouseEvent.MOUSE_MOVED || id == MouseEvent.MOUSE_DRAGGED) {
                bean.dispatchEvent(new MouseEvent(this, id, System.currentTimeMillis(), modifiers, (Integer) get(
                        component, ":mousex"), (Integer) get(component, ":mousey"),
                        clickcount, popuptrigger));
            }
        } else if (("button" == classname) || ("checkbox" == classname) || ("togglebutton" == classname)) {
            if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_EXITED) || (id == MouseEvent.MOUSE_PRESSED)
                ...
                if (("button" == classname) && ((mousepressed == null) || (mousepressed == component))
                    ...
                    setCursor(Cursor.getPredefinedCursor((id == MouseEvent.MOUSE_ENTERED) ? Cursor.HAND_CURSOR
            ...
            if (editable && (part == null)) { // textfield area
                Image icon = null;
                int left = ((id == MouseEvent.MOUSE_PRESSED) && ((icon = getIcon(component, "icon", null)) != null)) ? icon
                        .getWidth(this)
                        : 0;
            ...
            processField(x, y, clickcount, id, component, false, ("passwordfield" == classname), 0);
        } else if ("textarea" == classname) {
            if (!processScroll(x, y, id, component, part)) {
            ...
            if (part == null) {
                ...
                if ((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_EXITED)
                        || (id == MouseEvent.MOUSE_PRESSED) || (id == MouseEvent.MOUSE_RELEASED)) {
                    if (id == MouseEvent.MOUSE_PRESSED) {
                        setFocus(component);
            ...
            Object selected = get(component, "selected");
            if (((id == MouseEvent.MOUSE_ENTERED) || (id == MouseEvent.MOUSE_EXITED)) && (part != null)
                    && (selected == null) && getBoolean(part, "enabled", true)) {
                repaint(component, classname, part);
            } else if ((part != null)
                    && ((selected == null) ? (id == MouseEvent.MOUSE_PRESSED)
            ...
            if (part != null) {
                if (((id == MouseEvent.MOUSE_ENTERED) || (id == DRAG_ENTERED)) && getBoolean(part, "enabled", true)) {
                    set(component, "selected", part);
                    popupMenu(component);
                    repaint(component, classname, part);
                } else if (id == MouseEvent.MOUSE_RELEASED) {
            ...
            if (part == "header") {
                if (id == MouseEvent.MOUSE_PRESSED) {
                    ...
                    if ((bounds.x != mx) || (bounds.y != my)) {
                        ...
                        repaint(component, Math.min(bounds.x, mx), Math.min(bounds.y, my), bounds.width
                                + Math.abs(mx - bounds.x), bounds.height + Math.abs(my - bounds.y));
                        bounds.x = mx;
                        bounds.y = my;
                    }
                }
            ...
            // close dialog button
                ...
                if (id == MouseEvent.MOUSE_PRESSED) {
                    ...
                    setCursor(Cursor.getPredefinedCursor((part == ":n") ? Cursor.N_RESIZE_CURSOR
                            : (part == ":ne") ? Cursor.NE_RESIZE_CURSOR : (part == ":e") ? Cursor.E_RESIZE_CURSOR
     ...
     *  
     */
    private void select(Object component, Object row, boolean recursive, boolean shiftdown, boolean controldown) {
        String selection = getString(component, "selection", "single");
        Object lead = null;
     ...
     *  
     */
    private void processField(int x, int y, int clickcount, int id, Object component, boolean multiline,
            boolean hidden, int left) {
        if (id == MouseEvent.MOUSE_PRESSED) {
            ...
            if (clickcount > 1) {
                String text = getString(component, "text", "");
                while ((caretstart > 0)
                        && ((clickcount == 2) ? Character.isLetterOrDigit(text.charAt(caretstart - 1)) : (text
                                .charAt(caretstart - 1) != '\n'))) {
                    caretstart--;
                }
                while ((caretend < text.length())
                        && ((clickcount == 2) ? Character.isLetterOrDigit(text.charAt(caretend)) : (text
                                .charAt(caretend) != '\n'))) {
                    caretend++;
                }
            }
    ...
    private int getCaretLocation(Object component, int x, int y, boolean multiline, boolean hidden) {
        Font currentfont = (Font) get(component, "font");
        FontMetrics fm = getFontMetrics((currentfont != null) ? currentfont : font);
        char[] chars = multiline ? ((char[]) get(component, ":text")) : getString(component, "text", "").toCharArray(); // update
                                                                                                                        // it
        int linestart = 0;
        if (multiline) {
    ...
    private boolean processSpin(Object component, Object part) {
        ...
        try {
            ...
            if ((part == "up") ? (itext + step <= getInteger(component, "maximum", Integer.MAX_VALUE))
    ...
    private boolean findComponent(Object component, int x, int y) {
        ...
        if ("combobox" == classname) {
            if (getBoolean(component, "editable", true) && (x <= bounds.width - block)) {
                Image icon = getIcon(component, "icon", null);
                insidepart = ((icon != null) && (x <= 2 + icon.getWidth(this))) ? "icon" : null;
            } else {
                insidepart = "down";
            }
        } else if ("bean" == classname) {
            set(component, ":mousex", x);
            set(component, ":mousey", y);
        } else if (":combolist" == classname) {
            if (!findScroll(component, x, y)) {
                y += getRectangle(component, ":view").y;
                for (Object choice = get(get(component, "combobox"), ":comp"); choice != null; choice = get(choice,
                        ":next")) {
                    Rectangle r = getRectangle(choice, "bounds");
                ...
                if (isResizable || hasAction) {
                    ...
                    if (0 < x && x < port.width && 0 < y && y < port.y - 1) {
                        ...
                        for (int i = 0; i < columnwidths.length; i++) {
                            ...
                            if (isResizable
                                    && ((x > left + width - 4 && x < left + width) || (i < columnwidths.length - 1
                                            && x >= left + width && x < left + width + 4))) {
    ...
    private void repaint(Object component, Object classname, Object part) {
        ...
        if (classname == "combobox") { // combobox down arrow
            ...
            if (part == "text") { // spinbox textfield content
                ...
                repaint(component, b.x + b.width - block, (part == "up") ? b.y : (b.y + b.height - b.height / 2),
                        block, b.height / 2);
            }
        }
     ...
     * get the input focus
     * 
     * @param component a focusable widget inside visible and enabled parents,
     *        and tabbedpane's selected tab
    ...
    private boolean setNextFocusable(Object current, boolean outgo) {
        ...
        for (Object next = null, component = current; true; component = next) {
            ...
            while (next == null) { // find the next of the parents, or the
                ...
                if ((component == content)
                        || ((getClass(component) == "dialog") && (!outgo || getBoolean(component, "modal", false)))) {
     ...
     */
    private Object getPreviousFocusable(Object component, Object block, boolean start, boolean upward,
            boolean backward, boolean outgo) {
        Object previous = null;
     ...
     * @param forced splitpane is also checked (e.g. false for tab navigating,
     *        and true for mouse selection or application request)
    ...
    private boolean isFocusable(Object component, boolean forced) {
        ...
        if ((classname == "button") || (classname == "checkbox") || ("togglebutton" == classname)
                || (classname == "combobox") || (classname == "textfield") || (classname == "passwordfield")
                || (classname == "textarea") || (classname == "spinbox") || (classname == "slider")
    ...
    public static Object get(Object component, Object key) {
        if (useVectors) {
            if (key == ":comp") {
                ...
                if (slotInteger != null) {
                    int nextSlot = (Integer) get(component, ":slot") + 1;
     ...
     * 
     * @param component a widget (combobox, tabbedpane, list, table, header, or
     *        tree)
     ...
     *        insert the component at the end
     */
    ...
    private void insertItem(Object parent, Object key, Object component, int index) {
        if (useVectors && key.equals(":comp")) {
            ...
            if (index == -1) {
                set(component, ":slot", v.size());
                v.addElement(component);
            } else {
                set(component, ":slot", index);
     ...
     */
    private boolean checkMnemonic(Object component, boolean parent, Object checked, int keycode, int modifiers) {
        if ((component == null) || !getBoolean(component, "visible", true) || !getBoolean(component, "enabled", true)) { //+
                                                                                                                         // enabled
                                                                                                                         // comp
                                                                                                                         // in
            // disabled parent
        ...
        // check subcomponents
        if (("panel" == classname) || ("desktop" == classname) || ("dialog" == classname) || ("splitpane" == classname)
        ...
        if (parent && (("dialog" != classname) || !getBoolean(component, "modal", false))) {
            if (checkMnemonic(getParent(component), true, ("tab" == classname) ? checked : component, keycode,
                    modifiers)) {
                return true;
            }
        }
     ...
     * given xml resource
     * 
     * @param path is relative to your thinlet instance or the classpath (if the
     ...
     * handler
     * 
     * @param path is relative to your application package or the classpath, or
     *        an URL
     ...
     * 
     * @param resourcebundle a bundle for the next parsing or null to remove the
     *        current one
     ...
     * @throws java.lang.IllegalArgumentException
     */
    private Object parse(InputStream inputstream, boolean validate, boolean dom, Object handler) throws IOException {
            ...
            for (int c = reader.read(); c != -1;) {
                if (c == '<') {
                    if ((c = reader.read()) == '/') { //endtag
                        ...
                        while (">/ \t\n\r".indexOf(c) == -1) {
                            text.append((char) c);
                            if ((text.length() == 3) && (text.charAt(0) == '!') && (text.charAt(1) == '-')
                                    && (text.charAt(2) == '-')) {
                                int m = 0;
                                while (true) {
                                    c = reader.read();
                                ...
                                while (quote != (c = reader.read())) {
                                    if (c == '&') {
                                        ...
                                        if ("lt".equals(entity)) {
                                            ...
                                            text.append((char) Integer.parseInt(entity.substring(hexa ? 2 : 1),
                                                    hexa ? 16 : 10));
    ...
    private void addImpl(Object parent, Object component, int index) {
        ...
        if ((("combobox" == parentclass) && ("choice" == classname))
                ...
                || ((("menu" == parentclass) || ("popupmenu" == parentclass)) && (("menu" == classname)
                        || ("menuitem" == classname) || ("checkboxmenuitem" == classname) || ("separator" == classname)))
                || ((("panel" == parentclass) || ("desktop" == parentclass) || ("splitpane" == parentclass)
    ...
    public void setInteger(Object component, String key, int value) {
        ...
        if (setInteger(component, (String) definition[1], value, (Integer) definition[3])) {
            update(component, definition[2]);
        }
    }
    ...
    public int getInteger(Object component, String key) {
        return (Integer) get(component, key, "integer");
    }
     ...
     * @param color custom color to use, or null to reset component to use
     *        default color
     */
    ...
    private void setKeystrokeImpl(Object component, String key, String value) {
        ...
        if (value != null) {
            ...
            try {
                ...
                keystroke = ((long) modifiers) << 32 | keycode;
            } catch (Exception exc) {
                throw new IllegalArgumentException(token);
            }
        }
     ...
     *  
     */
    public static Object get(Object component, String key, String type) {
     ...
     * @param value the method name and parameters (e.g. <i>foo(this, this.text,
     *        mybutton, mybutton.enabled) </i> for <i>public void foo(Object
     *        component, String text, Object mybutton, boolean enabled) </i>)
     * @param root the search starting component for name components in the
     *        arguments
    ...
    private void update(Object component, Object mode) {
        ...
        while (component != null) {
            ...
            if (("layout" == mode)
                    || (("validate" == mode) && (("list" == classname) || ("table" == classname)
    ...
    private boolean setString(Object component, String key, String value, String defaultvalue) {
        if (allI18n && (langResource != null) && ((key == "text") || (key == "tooltip"))) {
    ...
    private String getString(Object component, String key, String defaultvalue) {
        ...
        return (value == null) ? defaultvalue : getI18NString(component, key, (String) value);
    }
     ...
     * 
     * @param res resource bundle containing localized texts for "text" and
     *        "tooltip"
     */
     ...
     * 
     * @param res resource bundle containing default localized texts for "text"
     *        and "tooltip"
     */
     ...
     * 
     * @param val if "true", translate by default; if "false", do not translate
     *        by default.
     */
    ...
    private boolean setChoice(Object component, String key, String value, String[] values, String defaultvalue) {
        ...
        for (String value1 : values)
        {
            ...
            {
                return set(component, key, value1);
            }
        }
     ...
     *  
     */
    private void setRectangle(Object component, String key, int x, int y, int width, int height) {
     ...
     * 
     * @param path is relative to your thinlet instance or the classpath, or an
     *        URL
     * @param preload waits for the whole image if true, starts loading (and
     *        repaints, and updates the layout) only when required (painted, or
    ...
    static {
        ...
        dtd = new Object[] {
                ...
                "spinbox", "textfield",
                    new Object[][] {
                        { "integer", "minimum", null, Integer.MIN_VALUE},
                        { "integer", "maximum", null, Integer.MAX_VALUE},
