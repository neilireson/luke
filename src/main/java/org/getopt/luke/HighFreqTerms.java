      ...
      //default HighFreqTerms behavior
        for (TermStats term : terms)
        {
            System.out.printf("%s:%s %,d \n",
                    term.field, term.termtext.utf8ToString(), term.docFreq);
        }
    }
    else{
      TermStats[] termsWithTF = sortByTotalTermFreq(reader, terms);
        for (TermStats aTermsWithTF : termsWithTF)
        {
            System.out.printf("%s:%s \t totalTF = %,d \t doc freq = %,d \n",
                    aTermsWithTF.field, aTermsWithTF.termtext.utf8ToString(),
                    aTermsWithTF.totalTermFreq, aTermsWithTF.docFreq);
        }
    }
    reader.close();
  }
