...
public class Luke extends Thinlet implements ClipboardOwner {
  ...
  private void loadPlugins() {
    ...
    try {
      StringBuilder errors = new StringBuilder("Unable to load some plugins:");
      boolean failures = false;
        for (Object pluginClass : pluginClasses)
        {
            try
            {
                LukePlugin plugin = (LukePlugin) ((Class) pluginClass).getConstructor(new Class[0]).newInstance(
                );
  ...
  public LukePlugin getPlugin(String className) {
      for (LukePlugin plugin1 : plugins)
      {
          Object plugin = plugin1;
  ...
  public void actionCommit() {
    ...
    try {
      ...
      TreeMap<String,String> ud = new TreeMap<>(userData);
  ...
  private IndexWriter createIndexWriter() {
    try {
      ...
      if (mp instanceof LogMergePolicy) {
        mp.setNoCFSRatio(IndexGate.preferCompoundFormat(dir) ? 1.0 : 0.0);
      } else if (mp instanceof TieredMergePolicy) {
        cfg.getMergePolicy().setNoCFSRatio(IndexGate.preferCompoundFormat(dir) ? 1.0 : 0.0);
      }
  ...
  public void openIndex(String name, boolean force, String dirImpl, boolean ro,
    ...
    try {
      ...
      if (IndexWriter.isLocked(d)) {
        if (!ro) {
          if (force) {
            FSLockFactory.getDefault().obtainLock(d, IndexWriter.WRITE_LOCK_NAME).close();
          } else {
  ...
  private void initOverview() {
    try {
      ...
      TreeSet<String> fields = new TreeSet<>(fn);
      ...
      String divText = "N/A";
      if (ir instanceof DirectoryReader) {
        List<LeafReaderContext> readers = ir.leaves();
...
//    add(r1, c1);
    ...
    try {
      List<String> files = new ArrayList<>(si.files());
      Collections.sort(files);
      map.put("---files---", files.toString());
      if (si.info.getUseCompoundFile()) {                               
        String cfsFiles[] = si.info.getCodec().compoundFormat().getCompoundReader(si.info.dir, si.info, IOContext.READONCE).listAll();
        files.clear();
    ...
    try {
      ...
      map = new LinkedHashMap<>();
      List<String> flds = new ArrayList<>(fis.size());
  ...
  private void showFiles(Directory dir, List<IndexCommit> commits) throws Exception {
    ...
    List<String> uFiles = new ArrayList<>(files);
  ...
  public void clipTopTerms(Object tTable) {
    ...
    StringBuilder sb = new StringBuilder();
      for (Object row : rows)
      {
          TermStats ti = (TermStats) getProperty(row, "ti");
          if (ti == null)
          {
              continue;
          }
          sb.append(ti.docFreq).append("\t").append(ti.field).append("\t").append(ti.termtext.utf8ToString()).append("\n");
      }
  ...
  public void actionCleanup() {
    ...
    try {
      ...
      Object filesTable = find(dialog, "filesTable");
        ...
        {
            String fileName = (String) allFile;
            Object row = create("row");
            add(filesTable, row);
  ...
  public void toggleKeep(Object filesTable) {
    ...
    if (rows == null || rows.length == 0) return;
      for (Object row : rows)
      {
          Boolean Deletable = (Boolean) getProperty(row, "deletable");
          boolean deletable = Deletable != null ? Deletable : true;
          deletable = !deletable;
          putProperty(row, "deletable", deletable);
          Object[] cells = getItems(row);
  ...
  public void _actionCleanup(Object filesTable) {
    ...
    try {
      Object[] rows = getItems(filesTable);
        for (Object row : rows)
        {
            String name = (String) getProperty(row, "fileName");
            Boolean Deletable = (Boolean) getProperty(row, "deletable");
            boolean deletable = Deletable != null ? Deletable : true;
            if (!deletable)
            {
      ...
      if (noDel.size() > 0) {
        StringBuilder msg = new StringBuilder("Some files could not be deleted:");
          for (Object aNoDel : noDel)
          {
              msg.append("\n").append(aNoDel);
          }
        errMsg = msg.toString();
      ...
      public void run() {
        ...
        try {
          ...
          if (p instanceof LogMergePolicy) {
            p.setNoCFSRatio(useCompound ? 1.0 : 0.0);
            if (useCompound) {
              p.setNoCFSRatio(1.0);
            }
  ...
  public boolean actionEditAdd(Object editdoc) {
    ...
    Object[] tabs = getItems(editTabs);
      for (Object tab : tabs)
      {
          String name = getString(tab, "text");
          if (name.trim().equals(""))
          {
              continue;
          }
          Object fBoost = find(tab, "fBoost");
          Object fText = find(tab, "sText");
          if (fText == null)
          {
              fText = find(tab, "rText");
          }
          Object cbStored = find(tab, "cbStored");
          Object cbIndexed = find(tab, "cbIndexed");
          Object cbTokenized = find(tab, "cbTokenized");
          Object cbTVF = find(tab, "cbTVF");
          Object cbTVFo = find(tab, "cbTVFo");
          Object cbTVFp = find(tab, "cbTVFp");
          Object cbCmp = find(tab, "cbCmp");
          Object cbBin = find(tab, "cbBin");
          Object cbONorms = find(tab, "cbONorms");
          Object cbOTF = find(tab, "cbOTF");
  ...
  public void actionMLT(Object docNum, Object docTable) {
    ...
    try {
        Collection<String> var = Util.fieldNames(ir, true);
        mlt.setFieldNames(var.toArray(new String[var.size()]));
    } catch (Exception e) {
  ...
  private void _showDocFields(int docid, Document doc) {
    ...
    putProperty(table, "docNum", docid);
    if (doc == null) {
    ...
    setString(find("docNum1"), "text", String.valueOf(docid));
      for (String idxField : idxFields)
      {
          IndexableField[] fields = doc.getFields(idxField);
          if (fields.length == 0)
          {
              addFieldRow(table, idxField, null, docid, null);
              continue;
          }
          for (IndexableField field : fields)
          {
              addFieldRow(table, idxField, field, docid, (TFIDFSimilarity) sim);
          }
      }
    doLayout(table);
  }
    ...
    SlowThread st = new SlowThread(this) {
      public void execute() {
        try {
          ...
          Terms tfv = ar.getTermVector(DocId, fName);
          if (tfv == null) {
            ...
            for (IntPair tv : tvs)
            {
                ...
                if (ip.starts != null)
                {
                    ...
                    for (int k = 0; k < ip.starts.length; k++)
                    {
                        ...
                        sb.append(ip.starts[k]).append("-").append(ip.ends[k]);
                    }
                    setString(cell, "text", sb.toString());
                }
                add(r, cell);
  ...
  public void clipTV(Object vTable) {
    ...
    for (int i = 0; i < rows.length; i++) {
      ...
      sb.append(tf.cnt).append("\t").append(tf.text);
      ...
      if (tf.starts != null) {
        ...
        for (int k = 0; k < tf.starts.length; k++) {
          if (k > 0) sb.append(',');
          sb.append(tf.starts[i]).append("-").append(tf.ends[i]);
        }
      }
      sb.append("\n");
    }
  ...
  public void actionExamineNorm(Object table) throws Exception {
    ...
    if (ar != null) {
     try {         
       ...
       byte curBVal=0;
       if (nnorms!=null) {curBVal=(byte)nnorms.get(docNum);}
  ...
  public void displayNewNorm(Object dialog) {
    ...
    try {
      ...
      putProperty(dialog, "newNorm", newFVal);
      doLayout(dialog);
    } catch (Exception e) {
      // XXX eat silently
    }
  ...
  public void _showPayloads(Object dialog) {
    ...
    boolean warn = false;
      for (Object row : rows)
      {
          BytesRef payload = (BytesRef) getProperty(row, "payload");
          if (payload == null)
          {
              continue;
          }
          Object cell = getItem(row, 2);
          String curEnc = enc;
  ...
  public Analyzer createAnalyzer(Object srchOpts) {
    ...
    try {
      ...
      try {
        ...
        zeroArgV = null;
        try {
          zeroArg = Class.forName(sAType).getConstructor();
  ...
  public AccessibleHitCollector createCollector(Object srchOpts) throws Exception {
    ...
    if (getBoolean(ckNormRes, "selected")) {
      ...
      if (getBoolean(ckLimCount, "selected")) {
        ...
        return new IntervalLimitedCollector(lim, scoreRes);
      } else {
...
//      }
  ...
  private void _search(final Query q, final IndexSearcher is,
    ...
    SlowThread t = new SlowThread(this) {
      public void execute() {
        ...
        putProperty(sTable, "resNum", resNum);
  ...
  public void prevPage(Object sTable) {
    int resNum = (Integer) getProperty(sTable, "resNum");
    if (resStart == 0) {
  ...
  private void _createResultRow(int pos, int docId, float score, Object sTable) throws IOException {
    ...
    StringBuilder vals = new StringBuilder();
      for (String idxField : idxFields)
      {
          ...
          IndexableField[] values = doc.getFields(idxField);
          vals.setLength(0);
          boolean decodeErr = false;
          if (values != null)
          {
              for (int k = 0; k < values.length; k++)
              {
                  ...
                  try
                  {
                      v = dec.decodeStored(idxField, (Field) values[k]);
                  }
                  catch (Throwable e)
                  {
                      e.printStackTrace();
    ...
    Thread t = new Thread() {
      public void run() {
        try {
          ...
          is.setSimilarity(sim);
          Explanation expl = is.explain(q, docid);
  ...
  public void actionDesignSimilarity() {
    LukePlugin designer = null;
      for (LukePlugin plugin : plugins)
      {
          if (plugin.getClass().getName().equals("org.getopt.luke.plugins.SimilarityDesignerPlugin"))
          {
              designer = plugin;
              break;
          }
