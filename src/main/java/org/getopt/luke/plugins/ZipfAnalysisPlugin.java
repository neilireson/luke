    ...
    SlowThread st = new SlowThread(app) {
      public void execute() {
        try {
          ...
          ArrayList<TermCount> terms = new ArrayList<>();
          ...
          TermCount sortedTerms[] = terms.toArray(new TermCount[terms
              .size()]);
          ...
          buckets.add(currentBucket);
            for (TermCount sortedTerm : sortedTerms)
            {
                currentBucket.addTermDf(sortedTerm.df);
